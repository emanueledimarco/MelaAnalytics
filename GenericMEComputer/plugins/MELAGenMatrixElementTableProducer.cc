...
class MELAGenMatrixElementTableProducer : public edm::one::EDProducer<edm::one::WatchRuns, edm::one::SharedResources> {
    public:
        MELAGenMatrixElementTableProducer( edm::ParameterSet const & iConfig )  ;
        ~MELAGenMatrixElementTableProducer() override {} // nothing to do
...
MELAGenMatrixElementTableProducer::MELAGenMatrixElementTableProducer( edm::ParameterSet const & iConfig ) :
...
void MELAGenMatrixElementTableProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
    ...
    out->setDoc("MELA weights at LHE level");
    ...
    if (normalize_) {
        ...
        for (auto it = meBranches_.begin()+1, ed = meBranches_.end(); it != ed; ++it) {
            out->addColumnValue<float>(it->name(), it->normValue(norm), it->doc(), nanoaod::FlatTable::FloatColumn);
        }
    } else {
        for (const auto & me : meBranches_) {
            out->addColumnValue<float>(me.name(), me.value(), me.doc(), nanoaod::FlatTable::FloatColumn);
        }
    }
...
MELAGenMatrixElementTableProducer::MEBranch::MEBranch(const std::string & line) :
...
void MELAGenMatrixElementTableProducer::MEBranch::optimizeForNormValue(const MELAGenMatrixElementTableProducer::MEBranch & other) {
...
//
bool MELAGenMatrixElementTableProducer::runDecay() {
...
bool MELAGenMatrixElementTableProducer::runNLOVHApprox() {
    MELACandidate* melaCand = mela_->getCurrentCandidate();
    if (!melaCand) return false;
...
bool MELAGenMatrixElementTableProducer::runBestLOAssociatedV() {
    MELACandidate* melaCand = mela_->getCurrentCandidate();
    if (!melaCand) return false;
...
bool MELAGenMatrixElementTableProducer::runNLOVBFApprox() {
    MELACandidate* melaCand = mela_->getCurrentCandidate();
    if (!melaCand) return false;
...
bool MELAGenMatrixElementTableProducer::runBestLOAssociatedVBF() {
    MELACandidate* melaCand = mela_->getCurrentCandidate();
    if (!melaCand) return false;
...
DEFINE_FWK_MODULE(MELAGenMatrixElementTableProducer);
